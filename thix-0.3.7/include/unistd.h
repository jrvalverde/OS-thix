...
extern void*	__sbrk __P ((unsigned __increment));
extern void*	sbrk __P ((unsigned __increment));

extern int	nice __P ((int __priority));
extern int	ustat __P ((__dev_t __dev, struct ustat *__ubuf));

extern int	statfs __P ((const char *path, struct statfs *__fsbuf));
extern int	fstatfs __P ((int fd, struct statfs *__fsbuf));

extern int	kctrl __P ((struct kctrl *kc));

extern int	mount __P ((char *device_name,
			    char *fstype,
			    char *directory, 
			    int options));

extern int	umount __P ((char *device_name));
